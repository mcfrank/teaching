...
shorttitle        : "Classroom teaching as optimal communication"
...
author: 
  ...
  - name          : "Michael C. Frank"
    ...
    address       : "450 Serra Mall, Jordan Hall, Stanford, CA 94305"
...
keywords          : "classroom teaching, communication, bayesian modeling, class size,  formative assessment"
wordcount         : "3540"
...
Our work here is very related to a recent body of research in machine learning that has also looked efficient teaching and learning, including in multi-learner settings. As in our study, in this work, teachers and learners are simulated agents interacting through choosing examples relevant to a statistical learning problem (our "teaching games"). For example, @rafferty2016 used planning techniques to select efficient policies for teaching complex concepts. In addition, @melo2018 examined the case of a teacher teaching a single student, showing that adding assessment of the student's knowledge state leads to gains in efficiency. Most directly relevant to our own work, @zhu2017 adopt a minimax framework, examining the minimal number of examples necessary to ensure a particular level of performance in a heterogeneous set of learners. They show in this framework that classroom diversity is the key factor limiting teaching efficiency [a result also found in our initial modeling work, @frank2014, and replicated in our current simulations]. 

In our current study, the teaching game we study is extremely simple: a teacher attempts to communicate the weight of a biased coin via showing a series of individual coin flip outcomes (heads or tails). The key choice is what set of flips to show. Given unlimited teaching time, a long sequence of flips could allow the learner to approximate the coin's weight very closely. But with bounded time, the teacher must choose selectively and with respect to the students' prior beliefs. These priors -- which stand in for the combination of knowledge and ability differences that vary across students in real classrooms -- are the key factor in determining teaching effectiveness. If two students have very different priors, when they observe the same sequence of examples, they will infer that the teacher is trying to communicate very different weights. Thus, our analysis focuses on managing student variability by intervening on the teaching game. 
...
```{r fig.cap="\\label{fig:pareto_regimes} Pareto frontiers for three example cost function regimes (panels). Line types show a range of target concepts (only .5, .6, and .7 are shown to decrease the range of values plotted), and point size shows the number of teachers. Other plotting conventions are as in Figure 5."}
